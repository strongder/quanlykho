...
public class UpdatePhieuXuat extends JDialog implements ActionListener{
	...
	public UpdatePhieuXuat(JPanel panel, String maPhieu) {
	...
	private void submit() {
		if (table2.getModel().getRowCount() != 0) {	
			PhieuXuat px = PhieuXuatDAO.getInstance().selectById(maPhieu);
			px.setNguoiTao(txtNguoiTao.getText());
			String gia = txtTotal.getText().replace(",", ".");
			px.setTongTien(Double.parseDouble(gia));
			PhieuXuatDAO.getInstance().update(px);
			ArrayList<ChiTietPhieu> listCTP = ChiTietPhieuXuatDAO.getInstance().selectByMaPhieu(maPhieu);
			for (Product p : listTable2) {
				...
				ctp.setDonGia(p.getGiaXuat());

				if (found) {
					ChiTietPhieuXuatDAO.getInstance().update(ctp);
				}else {
			...
			((PhieuXuatView) panel).displayTable();
			displayTableSP(tableModel1);
		} else {
	...
	private double calculateTotal() {
		double totalValue = 0;
		for (Product p : listTable2) {
			totalValue += p.getGiaXuat() * p.getSoLuong();
		}
		return totalValue;
	}
